  ...
  return (
    ...
    <div className="w-full flex justify-center items-start px-4 h-full max-h-full ">
        <div className="flex flex-col items-start justify-center max-w-lg w-full bg-white md:shadow-xl rounded-2xl md:px-8 py-8 px-4 border border-slate-100 animate-slide-up">
            ...
            <LabelledInput label="Title" value={blog.title} placeholder="What's your post about?" onchange={(e)=>{
                setBlog({
                    ...blog,
            ...
            <div className="w-full relative mb-5">
              ...
              <textarea value={blog.content} placeholder="Write your thoughts here..." className="w-full border border-slate-300 px-4 py-3 focus:outline-none rounded-xl focus:border-primary-500 focus:ring-2 focus:ring-primary-100 transition-all duration-200 text-slate-800 bg-slate-50 focus:bg-white placeholder:text-slate-400 mb-2 h-[25vh] resize-none" onChange={(e)=>{
                  setBlog({
                      ...blog,
            ...
            <button 
              ...
              className={`w-full bg-primary-600 hover:bg-primary-700 hover:shadow-lg hover:-translate-y-0.5 px-6 py-3 rounded-xl text-white font-inter font-semibold text-lg transition-all duration-200 mb-4 ${isPublishing ? 'opacity-75 cursor-not-allowed hover:-translate-y-0 hover:shadow-none bg-primary-500 hover:bg-primary-500' : ''}`}
              onClick={async()=>{
                ...
                try {
                  ...
                  setMsg(res.data.msg)
                  setBlog({ title: '', content: '' }) // Reset form
